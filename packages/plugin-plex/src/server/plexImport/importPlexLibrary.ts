...
import {createPlexClient} from './plexClient'
import {
  plexOldId,
  mapPlexRating,
} from './mapEntities'
...
function emptyCounts(): PlexImportCounts {
  return {
    people: 0,
    genres: 0,
    studios: 0,
...
function importChapters(
  ...
  for (const chapter of chapters) {
    const time = Math.round(chapter.startSeconds)
...
export async function importPlexLibrary(
  db: ApiDb,
  options: PlexImportOptions,
  onProgress?: PlexImportProgressCallback,
  isAborted?: () => boolean,
  clientFactory: typeof createPlexClient = createPlexClient,
  ...
  let snapshot: PlexLibrarySnapshot
  try {
  ...
  for (let i = 0; i < snapshot.items.length; i++) {
    ...
    const itemOldId = plexOldId('item', item.id)
    ...
    try {
      ...
      mediaRepo.updateById(mediaRow.id, {
        ...
        rating: mapPlexRating(item.rating),

...
export function isSortCompatibleWithGroupBy(groupBy: ItemsGroupBy, sortBy: unknown): boolean {
  ...
  if (REBUCKET_GROUP_BY.has(groupBy)) return true

  switch (groupBy) {
    case 'firstLetter':
      return String(sortBy) === 'name'
    case 'dateMonth':
    case 'dateYear':
    case 'dateDay':
      return isDateGroupSortField(sortBy)
    case 'rating':
      return String(sortBy) === 'rating'
    case 'path':
    case 'diskRoot':
      return String(sortBy) === 'path'
    case 'filesize':
      return String(sortBy) === 'filesize'
    case 'duration':
      return String(sortBy) === 'duration'
    case 'views':
      return String(sortBy) === 'views'
    case 'codec':
      return String(sortBy) === 'codec'
    case 'fps':
      return String(sortBy) === 'fps'
    case 'bitrate':
      return String(sortBy) === 'bitrate'
    case 'resolution':
      return String(sortBy) === 'height' || String(sortBy) === 'width'
    default:
      return false
  }
}
...
function compareGroupKeys(groupBy: ItemsGroupBy, a: string, b: string): number {
  if (a === b) return 0
  if (a === '#') return 1
  if (b === '#') return -1

  if (groupBy === 'favorite') return a === '1' ? -1 : 1
...
function buildRebucketedGroups<T extends GroupableItem>(
  ...
  return [...buckets.entries()]
    .sort(([keyA], [keyB]) => compareGroupKeys(groupBy, keyA, keyB))
...
export function buildItemGroups<T extends GroupableItem>(
  ...
  if (!isSortCompatibleWithGroupBy(active, sortBy) && !REBUCKET_GROUP_BY.has(active)) {
    return []
  }

  if (REBUCKET_GROUP_BY.has(active)) {
    return buildRebucketedGroups(items, active, sortBy, resolvedOptions)
  }

  const sections: ItemsGroupSection<T>[] = []
  for (const item of items) {
    const {key, label, filter} = getGroupKeyAndLabel(item, active, sortBy, resolvedOptions)
    const last = sections[sections.length - 1]
    if (last && last.label === label) last.items.push(item)
    else sections.push({key: `${key}-${sections.length}`, label, items: [item], filter: filter ?? null})
  }
  return sections
}

...
// Keep dimming under app bar / sticky items deck / nav chrome (~950–1000).
...
const updateSpotlight = async () => {
  ...
  // Keep the spotlight hole below sticky page chrome so it cannot punch
  // through the items control deck / app bar while the card is scrolled under.
  const chromeBottom = Math.max(
    document.querySelector('.v-app-bar')?.getBoundingClientRect().bottom ?? 0,
    document.querySelector('.items-control-deck')?.getBoundingClientRect().bottom ?? 0,
  )
  const rect = spotlightRect.value
  if (rect.top < chromeBottom) {
    const clipped = chromeBottom - rect.top
    rect.top = chromeBottom
    rect.height = Math.max(0, rect.height - clipped)
  }
  if (rect.height < 8) {
    clearSpotlight()
  }

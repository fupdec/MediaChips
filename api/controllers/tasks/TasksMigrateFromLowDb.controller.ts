...
import { apiErrorMessage, sendAsClientError, sendOk } from '../../types/errors'
import fs from 'fs'
...
export default function (db: ApiDb) {
  ...
  const cleanDataLowDb = async (req: ApiRequest, res: ApiResponse) => {
    try {
      ...
      console.error(err)
      sendAsClientError(res, err, 'Failed to clear legacy LowDB data')
    }
  }
  ...
  const createBackupLowDb = async (req: ApiRequest, res: ApiResponse) => {
      ...
      if (req.body.is_copy_backups) {
        ...
        if (fs.existsSync(pathUserData)) {
          try {
            ...
            console.error(err)
            sendAsClientError(res, err, 'Failed to copy legacy backups')
            return
          }
        }
      }
    ...
    archive.on("error", (err: unknown) => {
      console.error(err);
      sendAsClientError(res, err, 'Failed to create LowDB backup archive')
    });
    archive.pipe(output);

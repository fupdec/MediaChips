...
const packageDir = join(root, 'plugins/official/jellyfin')
const outDir = join(root, 'plugins/dist')
const mainEntrySource = join(root, 'packages/plugin-jellyfin/src/server/registerPluginMain.ts')
...
if (build.status !== 0 || !existsSync(mainOut)) {
  console.error('Failed to bundle jellyfin plugin main.cjs')
  process.exit(build.status ?? 1)
}

const result = spawnSync(
  'zip',
  ['-r', zipPath, 'plugin.json', 'README.md', 'main.cjs'],
  {cwd: packageDir, stdio: 'inherit'},
)

if (result.status !== 0) {
  console.error('Failed to build jellyfin plugin zip (is `zip` installed?)')

...
const packageDir = join(root, 'plugins/official/emby')
const outDir = join(root, 'plugins/dist')
const mainEntrySource = join(root, 'packages/plugin-emby/src/server/registerPluginMain.ts')
...
if (build.status !== 0 || !existsSync(mainOut)) {
  console.error('Failed to bundle emby plugin main.cjs')
...
if (result.status !== 0) {
  console.error('Failed to build emby plugin zip (is `zip` installed?)')

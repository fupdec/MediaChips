...
const packageDir = join(root, 'plugins/official/stash')
const outDir = join(root, 'plugins/dist')
const mainEntrySource = join(root, 'packages/plugin-stash/src/server/registerPluginMain.ts')
...
if (build.status !== 0 || !existsSync(mainOut)) {
  console.error('Failed to bundle stash plugin main.cjs')
...
if (result.status !== 0) {
  console.error('Failed to build stash plugin zip (is `zip` installed?)')
